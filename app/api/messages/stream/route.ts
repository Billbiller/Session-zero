...
// app/api/notifications/stream/route.ts exactly, one level down (direct
// messages instead of general notifications) -- see that route and
// lib/messageEvents.ts's doc comment for why these stay two separate
// streams/counters rather than one combined one.
...
export async function GET(request: NextRequest) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      send("unread", { unreadCount: getTotalUnreadMessageCount(userId) });

      const unsubscribe = subscribeToUnreadMessageCount(userId, (evt) => {
        send("unread", evt);
      });
